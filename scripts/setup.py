        ...
        if can_auto_install:
            ...
            if ask_yes_no("Install missing prerequisites now?", True):
                ...
                if install_script.exists():
                    ...
                    print_warning("Installation script not found at: install.sh")
        ...
        if docker_started:
            ...
            if dashboard_file.exists():
                ...
                if success:
                    ...
                    print_info(f"{Colors.BOLD}Step 1: Verify containers are running{Colors.ENDC}")
                    print(f"  {Colors.BOLD}docker ps{Colors.ENDC}")
                    print("")
                    print_info("  You should see 3 containers:")
                    print("    • xrpl-dashboard-grafana")
                    print("    • xrpl-dashboard-prometheus")
                    print("    • xrpl-dashboard-node-exporter")
                    print("")

                    print_info(f"{Colors.BOLD}Step 2: Access the dashboard{Colors.ENDC}")
                    print("")
                    print_info("  If running on a REMOTE server, set up SSH tunnel from your local machine:")
                    print(f"    {Colors.BOLD}ssh -i <your-key> -L {grafana_port}:localhost:{grafana_port} user@server-ip{Colors.ENDC}")
                    print("")
                    print_info("  If running LOCALLY, skip the tunnel and access directly.")
                    print("")

                    print_info(f"{Colors.BOLD}Step 3: Open dashboard in your browser{Colors.ENDC}")
                    ...
                    print(f"  View containers:    {Colors.BOLD}docker ps{Colors.ENDC}")
                else:
